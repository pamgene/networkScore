...
See [`networkGen`'s README](../networkGen#local-development-notes) -- dev
commands must run under whichever R install has a real, compiled `PCSF`
(check for a `libs/` dir), not necessarily bare `Rscript`/`R`.
